...
$order_id = $_POST['order_id'];
    ...
    if (mysqli_num_rows($result) < 1) {
        ...
        /*Find client by client id from order details*/
        $client = select_by_id('my_coffee_clients', 'client_id', $row['order_client_id']);

        $result = "<h3 style='margin-top:0;margin-bottom: 17px;'>Замовлення № {$row['order_id']}</h3>";
        $result .= "<h3 style='margin-top:0;margin-bottom: 17px;font-size:16px'>Клієнт: {$client['client_name']}</h3>";
        ...
        $result .= "<thead><tr><th><span style='font-size: 16px; font-weight:bold;'>Замовлення: </span></th></tr></thead>";
        ...
        $result .= "<tr><td style='text-align:left; font-size:16px;'><b><u>Загальна вартість замовлення: " . number_format($row['order_sum'], 2, '.', ' ') . " грн.</u></b></td></tr>";
        $result .= "</table></div></div>";
        $result .= "<div style='position: relative;' class=\"panel-footer\">";

        $order_status = ($row['order_status'] == 1) ? "<span style='color: darkgreen;'>Виконане</span>" : "<span style='color: darkred;'>Не виконане</span>";


        $result .= "<p style=\"font-size: 16px; text-align:left; font-weight:bold; margin-bottom:0;\">Статус замовлення: {$order_status} </p>";

        if($row['order_status'] != 1){
            $result .= "<div class='row'><div class='col-xs-12'><a class='btn btn-danger btn-sm btn-to-arch' href='orders.php?to_archive=true&order_id={$order_id}'><i class='far fa-trash-alt'></i> В архів</a></div></div>";
        }
